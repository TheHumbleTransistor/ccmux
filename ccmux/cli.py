...
def attach(
    *,
    ...
    # Check if tmux session still exists (use session name, not stale $N ID)
...
def activate(
    ...
    *,
    ...
    # Check which worktrees are inactive (session-scoped)
    inactive_worktrees = []
    for wt in worktrees:
    ...
    if name is None:
        ...
        # Check if tmux session exists (use session name, not stale $N ID)
